...
export const patientService = {
  ...
  getMyProfile: async () => {
    const { data } = await api.get<PatientProfileResponseDto>(
      '/patients/me/profile',
    );
    return data;
  },
  ...
  getPatientById: async (patientId: string) => {
    const { data } = await api.get<PatientResponseDto>(
      `/patients/${patientId}`,
    );
    return data;
  },
  ...
  updatePatient: async (patientId: string, payload: UpdatePatientDto) => {
    const { data } = await api.patch<PatientResponseDto>(
      `/patients/${patientId}`,
      payload,
    );
    return data;
  ...
  getPatientMedicalRecords: async (patientId: string) => {
    const { data } = await api.get<MedicalRecordResponseDto[]>(
  ...
  getPatientVitalSigns: async (patientId: string) => {
    const { data } = await api.get<VitalSignResponseDto[]>(
  ...
  getPatientPrescriptions: async (patientId: string) => {
    const { data } = await api.get<PrescriptionResponseDto[]>(
